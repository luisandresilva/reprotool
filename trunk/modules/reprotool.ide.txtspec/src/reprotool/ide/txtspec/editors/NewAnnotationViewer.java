 ...
 * <p>
 * NOTE: The <code>NewAnnotationViewer</code> only supports projections that cover full lines.
 * </p>
 * <p>
...
public class NewAnnotationViewer extends SourceViewer implements ITextViewerExtension5 {
	...
	private class AnnotationModelListener implements IAnnotationModelListener, IAnnotationModelListenerExtension {
		...
		private void processModelChanged(IAnnotationModel model, AnnotationModelEvent event) {
			if (model == fNewAnnotationModel) {

				if (fNewAnnotationSummary != null)
					fNewAnnotationSummary.updateSummaries();
				processCatchupRequest(event);

			} else if (model == getAnnotationModel() && fNewAnnotationSummary != null)
				fNewAnnotationSummary.updateSummaries();
		}
	}
	...
	/**
	 * A command representing a change of the project	if (fgExpandedImage != null) {
//				fgExpandedImage.dispose();
//				fgExpandedImage= null;
//			}ion document. This can be either
	...
	private NewAnnotationModel fNewAnnotationModel;
	...
	private NewAnnotationSummary fNewAnnotationSummary;
	 ...
	 */
	public NewAnnotationViewer(Composite parent, IVerticalRuler ruler, IOverviewRuler overviewRuler, boolean showsAnnotationOverview, int styles) {
		super(parent, ruler, overviewRuler, showsAnnotationOverview, styles);
	}

	public NewAnnotationViewer(Composite container, IVerticalRuler ruler, int vScroll) {
		super(container,ruler,vScroll);
	}
	 ...
	 */
	public void setProjectionSummary(NewAnnotationSummary projectionSummary) {
		fNewAnnotationSummary= projectionSummary;
	}
	 ...
	 */
	private void addNewAnnotationModel(IAnnotationModel model) {
		if (model instanceof IAnnotationModelExtension) {
			IAnnotationModelExtension extension= (IAnnotationModelExtension) model;
			extension.addAnnotationModel(NewAnnotationSupport.NEW_ANNOTATION, fNewAnnotationModel);
	 ...
	 */
	private IAnnotationModel removeNewAnnotationModel(IAnnotationModel model) {
		if (model instanceof IAnnotationModelExtension) {
			model.removeAnnotationModelListener(fAnnotationModelListener);
			IAnnotationModelExtension extension= (IAnnotationModelExtension) model;
			return extension.removeAnnotationModel(NewAnnotationSupport.NEW_ANNOTATION);
		}
		return null;
	}
	...
	public void setDocument(IDocument document, IAnnotationModel annotationModel, int modelRangeOffset, int modelRangeLength) {
		...
		if (fNewAnnotationModel != null) {
			wasProjectionEnabled= removeNewAnnotationModel(getVisualAnnotationModel()) != null;
			fNewAnnotationModel= null;
		}
	...
	protected IAnnotationModel createVisualAnnotationModel(IAnnotationModel annotationModel) {
		...
		fNewAnnotationModel= new NewAnnotationModel();
		return model;
	}
	 ...
	 */
	public NewAnnotationModel getNewAnnotationModel() {
		IAnnotationModel model= getVisualAnnotationModel();
		if (model instanceof IAnnotationModelExtension) {
			IAnnotationModelExtension extension= (IAnnotationModelExtension) model;
			return (NewAnnotationModel) extension.getAnnotationModel(NewAnnotationSupport.NEW_ANNOTATION);
		}
		return null;
	}
	...
	public final boolean isProjectionMode() {
		return getNewAnnotationModel() != null;
	}
	...
	public final void disableProjection() {
		if (isProjectionMode()) {
			removeNewAnnotationModel(getVisualAnnotationModel());
			fNewAnnotationModel.removeAllAnnotations();
	...
	public final void enableProjection() {
		if (!isProjectionMode()) {
			addNewAnnotationModel(getVisualAnnotationModel());
			fFindReplaceDocumentAdapter= null;
	...
	private void expandAll() {
		...
		if (isProjectionMode()) {
			fNewAnnotationModel.expandAll(offset, length);
		}
	}
	...
	private void expand() {
		if (isProjectionMode()) {
			...
			for (Iterator e= fNewAnnotationModel.getAnnotationIterator(); e.hasNext();) {
				NewAnnotation annotation= (NewAnnotation) e.next();
				if (annotation.isCollapsed()) {
					Position position= fNewAnnotationModel.getPosition(annotation);
			...
			if (bestMatch != null) {
				fNewAnnotationModel.expand(bestMatch);
	...
	private void collapse() {
		if (isProjectionMode()) {
			...
			if (bestMatch != null) {
				fNewAnnotationModel.collapse(bestMatch);
				revealRange(selection.x, selection.y);
			}
		}
	}
	...
	private void collapseAll() {
		...
		int length= doc == null ? 0 : doc.getLength();
		if (isProjectionMode()) {
			fNewAnnotationModel.collapseAll(offset, length);
		}
	}
	...
	private void expand(int offset, int length, boolean fireRedraw) throws BadLocationException {
		...
		if (slave instanceof ProjectionDocument) {
			...
			NewAnnotation[] collapsed= computeCollapsedNestedAnnotations(offset, length);
			if (collapsed != null) {
				for (int i= 0; i < collapsed.length; i++) {
					IRegion[] regions= computeCollapsedRegions(fNewAnnotationModel.getPosition(collapsed[i]));
					if (regions != null)
	...
	protected final void processCatchupRequest(AnnotationModelEvent event) {
		if (Display.getCurrent() != null) {
			...
			if (run) {
				...
				try {
					catchupWithNewAnnotationModel(event);
				} catch (BadLocationException x) {
							...
							public void run() {
								try {
									while (true) {
										...
										catchupWithNewAnnotationModel(ame);
									}
								} catch (BadLocationException x) {
									try {
										catchupWithNewAnnotationModel(null);
	 ...
	 */
	private void catchupWithNewAnnotationModel(AnnotationModelEvent event) throws BadLocationException {
	...
	private NewAnnotation[] computeCollapsedNestedAnnotations(int offset, int length) {
		List annotations= new ArrayList(5);
		Iterator e= fNewAnnotationModel.getAnnotationIterator();
		while (e.hasNext()) {
			NewAnnotation annotation= (NewAnnotation) e.next();
			if (annotation.isCollapsed()) {
				Position position= fNewAnnotationModel.getPosition(annotation);
		...
		if (annotations.size() > 0) {
			NewAnnotation[] result= new NewAnnotation[annotations.size()];
	...
	private void processDeletions(AnnotationModelEvent event, Annotation[] removedAnnotations, boolean fireRedraw) throws BadLocationException {
		for (int i= 0; i < removedAnnotations.length; i++) {
			NewAnnotation annotation= (NewAnnotation) removedAnnotations[i];
			if (annotation.isCollapsed()) {
	...
	private void processChanges(Annotation[] annotations, boolean fireRedraw, List coverage) throws BadLocationException {
		for (int i= 0; i < annotations.length; i++) {
			NewAnnotation annotation= (NewAnnotation) annotations[i];
			Position position= fNewAnnotationModel.getPosition(annotation);
	...
	public boolean exposeModelRange(IRegion modelRange) {
		if (isProjectionMode())
			return fNewAnnotationModel.expandAll(modelRange.getOffset(), modelRange.getLength());
	...
	public void setRangeIndication(int offset, int length, boolean moveCursor) {
		...
		if (moveCursor && fNewAnnotationModel != null && (rangeIndication == null || offset != rangeIndication.getOffset() || length != rangeIndication.getLength())) {
			List expand= new ArrayList(2);
			// expand the immediate effected collapsed regions
			Iterator iterator= fNewAnnotationModel.getAnnotationIterator();
			while (iterator.hasNext()) {
				NewAnnotation annotation= (NewAnnotation)iterator.next();
				if (annotation.isCollapsed() && willAutoExpand(fNewAnnotationModel.getPosition(annotation), offset, length))
			...
			if (!expand.isEmpty()) {
				...
				while (e.hasNext())
					fNewAnnotationModel.expand((Annotation)e.next());
			}
		}
			...
			try {
				...
				if (ProjectionDocumentEvent.PROJECTION_CHANGE == e.getChangeType()) {
					if (e.getLength() == 0 && replaceLength != 0)
						fNewAnnotationModel.expandAll(e.getMasterOffset(), e.getMasterLength());
					...
					try {
						...
						if (numberOfLines > 1 || fDeletedLines > 1)
							fNewAnnotationModel.expandAll(master.getOffset(), replaceLength);
					} catch (BadLocationException x) {
					}
				}
