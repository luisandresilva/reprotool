 ...
 * @since 3.0
 */
class NewAnnotationRulerColumn extends AnnotationRulerColumn {

	private NewAnnotation fCurrentAnnotation;
	 ...
	 */
	public NewAnnotationRulerColumn(IAnnotationModel model, int width, IAnnotationAccess annotationAccess) {
	 ...
	 * @param annotationAccess the annotation access
	 */
	public NewAnnotationRulerColumn(int width, IAnnotationAccess annotationAccess) {
	...
	protected void mouseClicked(int line) {
		...
		if (fMouseDownLine != line)
			return;
		NewAnnotation annotation= findAnnotation(line, true);
		if (annotation != null) {
			NewAnnotationModel model= (NewAnnotationModel) getModel();
	...
	protected void mouseDoubleClicked(int rulerLine) {
		...
		NewAnnotation annotation= findAnnotation(rulerLine, false);
		if (annotation != null) {
			NewAnnotationModel model= (NewAnnotationModel)getModel();
			model.toggleExpansionState(annotation);
		}
	 ...
	 */
	private NewAnnotation findAnnotation(int line, boolean exact) {

		NewAnnotation previousAnnotation= null;
		...
		if (model != null) {
			...
			while (e.hasNext()) {
				Object next= e.next();
				if (next instanceof NewAnnotation) {
					NewAnnotation annotation= (NewAnnotation) next;
	 ...
	 */
	private int getDistance(NewAnnotation annotation, Position position, IDocument document, int line) {
	...
	public Control createControl(CompositeRuler parentRuler, Composite parentControl) {
		...
		control.addMouseMoveListener(new MouseMoveListener() {
			public void mouseMove(MouseEvent e) {
				...
				NewAnnotation annotation= findAnnotation(toDocumentLineNumber(e.y), false);
	...
	public void setModel(IAnnotationModel model) {
		if (model instanceof IAnnotationModelExtension) {
			...
			model= extension.getAnnotationModel(NewAnnotationSupport.NEW_ANNOTATION);
		}
