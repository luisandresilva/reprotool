...
/**
 * Supports the configuration of projection capabilities a {@link org.eclipse.jface.text.source.projection.NewAnnotationViewer}.
 * <p>
 ...
 */
public class NewAnnotationSupport {
	 ...
	 */
	public final static Object NEW_ANNOTATION= new Object();
	...
	private static class ProjectionAnnotationsPainter extends AnnotationPainter {
		...
		protected IAnnotationModel findAnnotationModel(ISourceViewer sourceViewer) {
			if (sourceViewer instanceof NewAnnotationViewer) {
				NewAnnotationViewer projectionViewer= (NewAnnotationViewer) sourceViewer;
				NewAnnotationModel annotationModel;
				IAnnotationModel model= projectionViewer.getVisualAnnotationModel();	
		...
		protected boolean skip(Annotation annotation) {
			if (annotation instanceof NewAnnotation)
				return !((NewAnnotation) annotation).isCollapsed();
	...
	private static class ProjectionDrawingStrategy implements AnnotationPainter.IDrawingStrategy {
		...
		public void draw(Annotation annotation, GC gc, StyledText textWidget, int offset, int length, Color color) {
			if (annotation instanceof NewAnnotation) {
				NewAnnotation projectionAnnotation= (NewAnnotation) annotation;
	...
	private NewAnnotationViewer fViewer;
	...
	private NewAnnotationRulerColumn fColumn;
	 ...
	 */
	public NewAnnotationSupport(NewAnnotationViewer viewer, 
	...
	protected void doEnableProjection() {
		...
		if (fPainter == null) {
			...
			fPainter.addDrawingStrategy(NEW_ANNOTATION, getDrawingStrategy());
			fPainter.addAnnotationType(NewAnnotation.TYPE, NEW_ANNOTATION);
			fPainter.setAnnotationTypeColor(NewAnnotation.TYPE, fSharedTextColors.getColor(getColor()));
			fViewer.addPainter(fPainter);
		}

		if (fColumn == null) {
			fColumn= new NewAnnotationRulerColumn(9, fAnnotationAccess);
			fColumn.addAnnotationType(NewAnnotation.TYPE);
	...
	private NewAnnotationSummary createProjectionSummary() {
		NewAnnotationSummary summary= new NewAnnotationSummary(fViewer, fAnnotationAccess);
		if (fSummarizableTypes != null) {
	 ...
	 *
	 */
	public Object getAdapter(ISourceViewer viewer, @SuppressWarnings("rawtypes") Class required) {
		if (NewAnnotationModel.class.equals(required)) {
			if (viewer instanceof NewAnnotationViewer) {
				NewAnnotationViewer projectionViewer= (NewAnnotationViewer) viewer;
				NewAnnotationModel annotationModel;
				IAnnotationModel model= projectionViewer.getVisualAnnotationModel();	
				if (model instanceof IAnnotationModelExtension) {
					IAnnotationModelExtension extension= (IAnnotationModelExtension) model;
